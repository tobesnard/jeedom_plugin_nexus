...
class Pylips:
            ...
            for api_version in possible_api_versions:
                ...
                if r.status_code == 200:
                    if "api_version" in r.json():
                        self.config["TV"]["apiv"] = str(
                            r.json()["api_version"]["Major"]
                        )
                    else:
                        print(
        ...
        def on_message(client, userdata, msg):
            if str(msg.topic) == self.config["MQTT"]["topic_pylips"]:
                try:
                    ...
                    return print(
                        "Invalid JSON in mqtt message:", msg.payload.decode("utf-8")
                    )
    ...
    def mqtt_update_ambilight(self):
        ambilight_status = self.get(
            "ambilight/currentconfiguration", self.verbose, 0, False
        )
    ...
    def mqtt_update_ambihue(self):
        ambihue_status = self.run_command(
            "ambihue_status", None, self.verbose, False, False
        )
